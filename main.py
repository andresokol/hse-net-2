...
from as_net_hw2 import parsers, models, dumpers, visualize

DEMO_PATH = 'demo/Abvt.graphml.xml'


def floyd_warshall(graph: models.Graph):
...
def find_reserve_route(graph: models.Graph, a_id: int, b_id: int, black_list) -> models.Route:
    ...
    class PriorityDistance:
        ...
        def __add__(self, other):
            return PriorityDistance(self.ugliness + other.ugliness, self.dist_km + other.dist_km)
    ...
    excluded_edges = [(black_list[i - 1], black_list[i]) for i in range(1, len(black_list))]
    ...
    for edge in graph.edges:
        ...
        if edge.source_id in black_list:
            ugliness = 1
        new_edges[edge.source_id][edge.target_id] = PriorityDistance(ugliness, edge.length_km)
...
def extract_route(graph: models.Graph, a_id: int, b_id: int, dist, next_vertex) -> models.Route:
    route = models.Route()
    route.start_id = graph.nodes[a_id].node_id
    route.target_id = graph.nodes[b_id].node_id
    ...
    x_id = a_id
    while x_id != b_id:
        # route.path.append(graph.nodes[x_id].node_id)
        route.path.append(x_id)
        x_id = next_vertex[x_id][b_id]
    # route.path.append(graph.nodes[b_id].node_id)
...
def main(name: str, find_reserve: bool):
    ...
    for i in range(graph.node_cnt):
        for j in range(graph.node_cnt):
            if i == j:
                continue
            direct_route = extract_route(graph, i, j, dist, next_vertex)
            routes.append(direct_route)
            routes.append(find_reserve_route(graph, i, j, direct_route.path[1:-1]))
