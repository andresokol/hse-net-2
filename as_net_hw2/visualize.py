...
<body>
<script src="https://api-maps.yandex.ru/2.1/?apikey=73bc02d2-1232-4c7a-b4a7-5a58f210a2e0&lang=en_US"
...
def _vis_node(node: models.Node, is_selected):
    return f'''myMap.geoObjects.add(
    new ymaps.Placemark(
        [{node.lat}, {node.lon}],
        {{ balloonContent: "{node.label}" }},
        {{ preset: 'islands#circleIcon', iconColor: '{"#000000" if is_selected else "#3b599855"}' }}
    )
);'''


def _vis_edge(graph: models.Graph, edge: models.Edge):
...
def _vis_route(graph: models.Graph, route: models.Route, is_reserve=False):
    d = []
...
def visualize(graph: models.Graph, filename: str, route: models.Route,
