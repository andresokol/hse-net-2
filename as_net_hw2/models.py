...
class Node:
    ...
    def is_valid(self):
        return hasattr(self, 'node_id') and hasattr(self, 'lat') and hasattr(self, 'lon')
...
def _geo_to_radian(x: float) -> float:
    return x / 180.0 * pi


def _get_surface_dist_km(a: Node, b: Node) -> float:
    lat_rad_a = _geo_to_radian(a.lat)
    lon_rad_a = _geo_to_radian(a.lon)
    lat_rad_b = _geo_to_radian(b.lat)
    lon_rad_b = _geo_to_radian(b.lon)

    # According to https://en.wikipedia.org/wiki/Great-circle_distance
    # By default Python float should be 64-bit so this should work fine
    central_angle = acos(
...
class Graph:
    ...
    def __init__(self, nodes: tp.List[Node], connections: tp.List[tp.Tuple[str, str]]):
        self.nodes = nodes
        self.node_cnt = len(nodes)
        self.nodes_idx = {self.nodes[i].node_id: i for i in range(self.node_cnt)}
