...
def parse_graphml(path: str) -> models.Graph:
    ...
    for element in graph_element.getElementsByTagName('node'):
        ...
        if node.is_valid():
            ...
            print(f'Node with id {node.node_id} is invalid, skipping node and all adjacent edges')
    print(f'Parsed {len(nodes)} nodes')

    edges: tp.List[tp.Tuple[str, str]] = []
    for element in graph_element.getElementsByTagName('edge'):
        edges += [(element.getAttribute('source'), element.getAttribute('target'))]
        edges += [(element.getAttribute('target'), element.getAttribute('source'))]  # FIXME
