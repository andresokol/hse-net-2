...
TOPO_HEADERS = ['Node 1 (id)', 'Node 1 (label)', 'Node 1 (longitude)', 'Node 1 (latitude)',
                'Node 2 (id)', 'Node 2 (label)', 'Node 2 (longitude)', 'Node 2 (latitude)',
                'Distance (km)', 'Delay (mks)']

ROUTES_HEADERS = ['Node 1 (id)', 'Node 2 (id)', 'Path type', 'Path', 'Delay (mks)']
...
def dump_topo(graph: models.Graph, name: str):
    with open(f'{name}_topo.csv', 'w') as file:
        ...
        sorted_edges = sorted(graph.edges, key=lambda e: e.source_id * graph.node_cnt + e.target_id)
